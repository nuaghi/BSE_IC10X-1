***
      ...
      implicit none
      integer kw
      real*8 lum,r,mt,mc,rl,z,teff11
...
* for massive stars over the entire HRD.
      ...
      if(lum.gt.4000.d0)then
         ...
         teff11 = 1000.d0*((1130.d0*lum/r**2.d0)**(1.d0/4.d0))
         if(teff11.gt.1.25d4.and.teff11.le.2.5d4)then
         dms = 10**(-6.688+2.21*log10(lum/1.0d5)-1.339
     & *log10(mt/30.0)-1.601*log10(1.3/2.0)
     & +1.07*log10(teff11/2.0d4)+0.85*log10(z/0.02))
         elseif(teff11.gt.2.5d4.and.teff11.le.5.0d4)then
         dms = 10**(-6.697+2.194*log10(lum/1.0d5)-1.313
     & *log10(mt/30.0)-1.226*log10(2.6/2.0)
     & +0.933*log10(teff11/4.0d4)
     & -10.92*(log10(teff11/4.0d4))**2+0.85*log10(z/0.02))
         endif
...
* for high pulsation periods on AGB.
         ...
         if(kw.gt.6)then
            dms = MAX(dml,0.5*1.0d-13*hewind*
     &         lum**(3.d0/2.d0)*(z/0.02)**0.86)
c            dms = MAX(dml,0.5d-13*hewind*
c     &         lum**1.36*(z/0.02)**0.61)
         else
            dms = MAX(dml,dms)
...
c               dml = 0.1d0*(x-1.d0)**3*(lum/6.0d+05-1.d0)
               dml = 1.5d-4
               dms = dms + dml
            endif
         endif
