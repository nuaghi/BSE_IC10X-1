***
      SUBROUTINE evolv23(kstar,mass0,mass,rad,lumin,massc,radc,
...
*
      ...
      INTEGER kstar(2),kw,kst,kw1,kw2,kmin,kmax,ces
...
*
      ...
      REAL*8 porb6,porb8,porb10,porb12,q1,porb7,porb9,q2,tbr,porb15
      ...
      REAL*8 m1ce,m2ce,mch,tmsnew,dm22,mew,kkk,ospbru2
      ...
      REAL*8 neta,bwind,hewind,mxns,radmin,radmax,radrl,trl,qrl,qmax
...
*
               if(djmb.gt.tiny)then
                  dtj = 0.03d0*jspin(k)/ABS(djmb)    !!!!!!!!!!!!!!!!!!!!!   0.03
                  dt = MIN(dt,dtj)
               endif
            endif
*
...
*
         ...
         if(ABS(djtt).gt.tiny)then
            dtj = 0.002d0*jorb/ABS(djtt)    !!!!!!!!!!!!!!!!!!!!! 0.02
            dt = MIN(dt,dtj)
         endif
         dtm = dt/1.0d+06
*
...
*
            if(dms(k).gt.0.01d0*mass(k))then         !!!!!!!!!!!!!!!!!!!   0.01
...
*
c      pd = sep*(1.d0 - ecc)
c      if(pd.lt.(rad(1)+rad(2)).and.intpol.eq.0) goto 130
*
...
*
c      if(kstar(j1).eq.2)then
c         qc = 4.d0
       q1 = mass1i/mass2i
c      if(kstar(j2).eq.13.and.mass(j1).gt.6) qc = 1.d4      
***      
c      write(*,*)porb7,porb8,tbi

      if(kstar(j1).le.2.or.mass1i.ge.12.0.and.kstar(j1).eq.4)then
          porb15=0.548+0.0945*mass1i-0.001502*mass1i**2
     &        +1.0184d-5*mass1i**3-6.2267d-8*mass1i**4
          porb2=0.1958+0.3278*mass1i-0.01159*mass1i**2
     &        +0.0001708*mass1i**3-9.55d-7*mass1i**4
          porb25=6.0143+0.01866*mass1i-0.0009386*mass1i**2
     &          -3.709d-5*mass1i**3+5.9106d-7*mass1i**4
          porb3=24.6-1.85*mass1i+0.0784*mass1i**2
     &           -0.0015*mass1i**3+1.024d-5*mass1i**4
        if(mass1i.le.16.0)then
          porb35=1772.8-551.48*mass1i+66.14*mass1i**2
     &            -3.527*mass1i**3+0.07*mass1i**4
        else
          porb35=82.17-5.697*mass1i+0.17297*mass1i**2
     &            -0.00238*mass1i**3+1.206d-5*mass1i**4
       endif
        if(mass1i.le.16.0)then
          porb4=46511.2-10670*mass1i+919.88*mass1i**2
     &            -35.25*mass1i**3+0.506*mass1i**4
        else
          porb4=153.03-8.967*mass1i+0.2077*mass1i**2
     &            -0.00204*mass1i**3+6.677d-6*mass1i**4
        endif
        if(mass1i.le.24.0)then
          porb5=86434.6-15494.3*mass1i+1041.4*mass1i**2
     &            -31.017*mass1i**3+0.345*mass1i**4
         else
          porb5=566.5-33.123*mass1i+0.7589*mass1i**2
     &            -0.00776*mass1i**3+2.94d-5*mass1i**4    
        endif     

        if(mass1i.le.40.0)then
          porb6=219152.8-24416.3*mass1i+1018.7*mass1i**2
     &            -18.834*mass1i**3+0.1301*mass1i**4
         else
          porb6=-10744.14+856.43*mass1i-24.834*mass1i**2
     &            +0.3147*mass1i**3-0.00148*mass1i**4    
        endif     


      if(tbi.le.porb15)then
        if(q1.le.1.5)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb2)then
        if(q1.le.2)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb25)then
        if(q1.le.2.5)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb3)then
        if(q1.le.3)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb35)then
        if(q1.le.3.5)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb4)then
        if(q1.le.4)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb5)then
        if(q1.le.5)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.le.porb6)then
        if(q1.le.6)then
         qc = 1.d4
        else
         qc = 0
        endif
      elseif(tbi.gt.porb6)then
        if(q1.le.6)then
         qc = 1.d4
        else
         qc = 0
...
*        qc = 100.d0
      elseif(kstar(j1).ge.7.and.kstar(j1).le.9)then
c         qc = 0.784d0
c         qc = 10.0d0

        if(tb.le.1.644d-4)then
         qc = 0.1d0
        else
         qc = 10.0d0
        endif
...
*
      ...
      if(kstar(j1).ge.7.0.and.kstar(j1).le.9.0)then
        if(tb.le.1.644d-4)then
         qc = 0.1d0
        else
         qc = 10.0d0
...
c      if(mass(j2).ge.3.0.and.mass(j2).lt.7.5)then
c      porbup=700.0
c      elseif(mass(j2).ge.7.5.and.mass(j2).lt.15)then
c      porbup=1500.0
c      elseif(mass(j2).ge.15.and.mass(j2).lt.25)then
c      porbup=2500.0
c      elseif(mass(j2).ge.25)then
c      porbup=3500.0
c      endif

c      porblow=10**(0.7*mass(j1)/mass(j2)-1.0)

!      if(tb*yeardy.ge.porbup.and.tb*yeardy.le.(porbup+300.0)
!     &        .or.tb*yeardy.lt.porblow)then
!      qc=1.5
!      else
!      qc=1.0d4
!      endif
      ...
      if(trl.ge.-10.0)then
      radmax = -173.8+45.5*mass(j1)-0.18*mass(j1)**2.0   
      radmin = 6.6-26.1*mass(j1)/mass(j2)+11.4*(mass(j1)/mass(j2))**2.0
      radrl = rad(j1)

       if(radrl.gt.radmax)then
       qc = 0.0
       elseif(radrl.lt.radmin)then
       qc = 0.0
       else
       qc = 100.0
       endif
      ...
      qrl = mass(j1)/mass(j2)
       ...
       if(qrl.lt.2.0)then
       qc = 100.0
       elseif(qrl.gt.qmax)then
       qc = 0.0
       endif

c       print*,radmax,qrl,qmax,qc
...
c      print*,tb*yeardy,porbup,porblow,mass(j2)
...
*
         ...
         m1ce = mass(j1)
         m2ce = mass(j2)
         CALL comenv3(mass0(j1),mass(j1),massc(j1),aj(j1),jspin(j1),
...
*
         ospbru2 = twopi*SQRT(mass(2)*aursun**3/rad(2)**3)
         kkk = 1.0-ospin(2)/ospbru2
c        print*,ospbru2,mass(2),kkk 
            dm2 = MIN(1.0d0,kkk)*dm1                 ! ....................
...
c         if(kw.eq.12.and.mt.ge.1.38)then                          !......................
c         kw=13
c         mt=1.3
c         endif
...
*
     ...
     &               jspin(j2),kstar(j2),zpars,ecc,sep,jorb,coel,z)
      ...
      elseif(kstar(j2).ge.2.and.kstar(j2).le.9.and.kstar(j2).ne.7)then
         ces = ces + 1
         CALL comenv3(mass0(j2),mass(j2),massc(j2),aj(j2),jspin(j2),
