...
*
      ...
      COMMON f_fb,meanvk,sigmavk
*
      real*8 mass,aj,mt,tm,tn,tscls(20),lums(10),GB(10),zpars(20)
      real*8 r,lum,mc,rc,menv,renv,k2,a1,b1,a2,b2,mproto,SNtype
      real*8 mch,mlp,tiny
      parameter(mch=1.44d0,mlp=12.d0,tiny=1.0d-14)
      real*8 mass0,mt0,mtc
      REAL*8 neta,bwind,hewind,mxns,f_fb,mrem1,mrem2
...
*
      real*8 p1,p2,pcf,pbh,mnsmin,meanbh,sigmabh,meanvk,sigmavk
      real*8 thookf,tblf,normal,mean0,sigma0,m11,m22,m33,m44
...
* Use NS/BH mass given by Belczynski et al. 2002, ApJ, 572, 407. 
                     ...
                     if(SNtype.le.1.5.and.SNtype.ge.0.5)then   ! rapid SN
*
                     mproto = 1.0
...
c                        mcx = 0.161767d0*mc + 1.067055d0
                      mcx = 0.2
                     elseif(mc.lt.6.d0.and.mc.ge.2.5d0)then
                      mcx = 0.286*mc-0.514
c                        mcx = 0.314154d0*mc + 0.686088d0
                     elseif(mc.lt.7.d0.and.mc.ge.6.d0)then
                       mcx = mt-1.0
                     elseif(mc.lt.11.d0.and.mc.ge.7.d0)then
                       a1 = 0.25-1.275/(mt-mproto)
                       b1 = -11.0*a1+1.0
                       mcx = (mt-mproto)*(a1*mc+b1)
                     else
                       mcx = mt-mproto
                     endif
                     f_fb = mcx/(mt-mproto)
                   ...
                   mrem1 = -6.6667+0.6667*(100+30*mt)**0.5
                   mrem2 = 0.9*mt
                  if(mrem1.le.mxns)then
*
* Zero-age Neutron star
*
                     kw = 13
                  elseif(mrem2.ge.mxns)then
*
* Zero-age Black hole
*
                     kw = 14
                  ...
                  elseif(SNtype.gt.1.5.and.SNtype.le.2.5)then  !  delayed SN

                  if(mc.le.3.5)then
                  mproto = 1.2
                  elseif(mc.gt.3.5.and.mc.le.6.0)then
                  mproto = 1.3
                  elseif(mc.gt.6.0.and.mc.le.11.0)then
                  mproto = 1.4
                  elseif(mc.gt.11)then
                  mproto = 1.6
                  endif

                  if(mc.le.2.5)then
                  mcx = 0.2
                  elseif(mc.gt.2.5.and.mc.le.3.5)then
                  mcx = 0.5*mc-1.05
                  elseif(mc.gt.3.5.and.mc.le.11.0)then
                  a2 = 0.133-0.093/(mt - mproto)
                  b2 = -11.0*a2+1.0
                  mcx = (mt-mproto)*(a2*mc+b2)
                  elseif(mc.gt.11.0)then
                  mcx = mt-mproto
                  endif
                  f_fb = mcx/(mt-mproto)
                  mt = mcx + mproto
...
*
                  ...
                  elseif(SNtype.gt.2.5.and.SNtype.le.3.5)then  !  stochastic SN
                   mnsmin = 1.13
                   m11 = 2.0
                   m22 = 3.0
                   m33 = 7.0
                   ...
                   sigma0 = 0.02
                   kw = 13
                   mt = max(1.13,normal(mean0,sigma0))
                   elseif(mc.ge.m11.and.mc.lt.m22)then
                   ...
                   mt = max(2.0,normal(meanbh*mc,sigmabh))
                   endif
                   else
                   kw = 13
                   mt = max(1.13,normal(mean0,sigma0))
                   ...
                   elseif(mc.ge.m22.and.mc.lt.m33)then
                   ...
                   if(kw.eq.13)then
                   meanvk = 400.0*(mc-mt)/mt
                   sigmavk = 0.3*meanvk
                   elseif(kw.eq.14)then
                   meanvk = 200.0*max((mc-mt)/mt,0.0)
                   sigmavk = 0.3*meanvk
                   endif

                endif                

c                 print*, kw, p1, pbh,mcbagb
c                  mc = mt
c      write(10,*) mc, mt, kw
...
c                  print*,meanvk,sigmavk,mc
                  endif

                  endif

c                  write(10,*) SNtype,f_fb,normal(SNtype,SNtype)
c                  mc = mt
c      write(10,*) mc, mt, kw
