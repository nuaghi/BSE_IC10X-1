...
*
      ...
      real*8 mu,cmu,vs(3),v1,v2,mx1,mx2,f_fb,meanvk,sigmavk
      real*8 sigma,SNtype,normal,neta,bwind,hewind,mxns
      COMMON /VALUE4/ sigma,bhflag
      COMMON /VALUE1/ neta,bwind,hewind,mxns,SNtype
      COMMON f_fb,meanvk,sigmavk
      real ran3,xx
      external ran3, normal
*
c      write(*,*)m1n

      if (m1n .le. 1.301d0.and.m1n.gt.1.299)then
       sigma = 30.d0                              !........      
       else
       sigma = 265.d0
      endif       
...
* Generate two velocities from polar coordinates S & THETA.
 ...
 20   continue
      ...
      if(kw.eq.14)then       !.............
c      print*,f_fb, min(1.0,(m1n/0.9-1.0)/(m1-1.0)),m1
c       f_fb = min(1.0,(m1n/0.9-1.0)/(m1-1.0))
         vk2 = vk2*(1.0-f_fb)**2.0
         vk = SQRT(vk2)      
c        vk2 = 0.d0
c        vk = 0
      endif
c            write(*,*)m1n/0.9,m1,f_fb

      if(SNtype.gt.2.5.and.SNtype.le.3.5)then  !!  stochastic SN
      vk = normal(meanvk,sigmavk)
      vk = abs(vk)
      vk2 = vk**2.0
c      write(10,*)m1n,kw,vk
      endif
...
c         vst = sqrt(vs(1)**2+vs(1)**2+vs(3)**2)
*
 95   continue
*
      RETURN
